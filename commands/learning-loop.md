...
- 刷题模式（`*-drill/` 工作区）遵循 skill「Drill-mode flow」：纯 markdown 闭环，一次一题（每轮新建 `题目-NNN/`，选择/填空用户写进 md 作答区、算法题在同文件夹代码文件里实现）；掌握 = 累计答对 ≥3 次且最近一次正确；答错在后续随机轮次复现；全部真题掌握后基于真题生成变种加难，无上限；用户可随时补充真题（粘贴直接入库，链接派 Job 6 子代理采集）。
