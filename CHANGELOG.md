...
### 新增（刷题模式：题库驱动的随机刷题闭环，初始化时可选）

用户反馈：除系统化课程学习外，还需要一种"刷题"形态——初始化项目时可选刷题模式；刷题工作区只有一个核心初始目录「题库」；AI 每次构建一个文件夹、在文件夹内创建 md 写入题目，一次一道，用户做一个出一个；选择/填空直接写入 md 作答，算法题直接在当前题目文件夹里建代码文件实现；允许用户提供真题库链接由子代理自动采集，用户输入真题后按真题选题出题；每题至少出现三次且答对才算掌握，答错则在后续随机时段再次出现；全部真题练熟后基于真题生成变种提高难度检验水平，刷题空间无上限；过程中可随时补充真题。
...
- **每轮闭环（一次一题）**：随机选一题 → 新建题目文件夹 → 用户作答（选择/填空写 md、算法题写代码）→ 说「做好了」→ AI 读取批阅（算法题尽量实际运行最小用例验证）→ 批阅写回 `题目.md` + `meta.json` 计数 + `题库/index.md` 同步 → 出下一题，循环无上限。
- **掌握闭环（已与用户确认规则）**：每题随机选取、同题可重复但相邻两轮不重复；**掌握 = 累计答对 ≥3 次且最近一次作答正确**；答错则随机排期 `due_round` 在后续 2–5 轮内复现，复现再错继续随机复现。选题优先级：到期错题复现 > 未掌握真题 > 未掌握 AI 题 > 新变种。
- **变种加难**：全部真题（含链接采集）掌握后进入变种阶段——基于真题改条件/换角度/组合多题/边界加深，`variant_level` 逐档提升，变种入库同样进入掌握闭环；刷题空间（轮数/题库规模/变种档位）无上限。
- **题库随时可补**：任一轮用户说「补充真题」即可——粘贴文本由 AI 直接入库；真题库链接派发新增的 **Job 6 题库采集员**子代理抓取解析入库（采集后无法判定答案的题由 AI 补答案并标注后方可进入选题池；链接失效降级为粘贴路径，不阻断刷题）。
- **空题库启动**：初始化时用户暂无真题 → AI 按主题自主出题（约 5–8 题起步，易→难，来源标记 AI），真题随时补充后优先出真题。
- 同步更新：`SKILL.md`（版本 1.7.0、description 触发词、循环图双分支、First run/resume 扫描 `*-drill/`、目录布局/meta schema/初始化模式选择、新增「Drill-mode flow」章节、Subagents 六任务、Resume checklist 刷题分支、Communication style）、`references/templates.md`（FORMAT NOTE 注明刷题全 md + 新增「Drill-mode templates」：index.md / Q-xxx.md / 题目-NNN/题目.md / 算法代码文件头）、`references/quiz-types.md`（新增「刷题模式出题规则」：三题型、来源优先级、AI 出题质量、变种难度阶梯）、`references/subagent-protocol.md`（Job 6 题库采集员 + 任务表新增一行）、`commands/learning-loop.md`（分支 A/B/C/D 支持刷题、通用约束）、`README.md`（刷题模式简介）。
