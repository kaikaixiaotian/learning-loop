...
Schema 见 SKILL.md「meta.json — the state machine」末尾的 drill schema，此处不重复。要点：`mode: "drill"`、`round`、`lang`、`bank`（每题 `source/type/knowledge/shown/correct/last_round`——题目是载体不是跟踪单位）、`knowledge` 知识点掌握表（`stage/status/due_round/attempts/correct/last_round`——跟踪与掌握单位是知识点，掌握由 AI 判断）、`variant_level`、`history`。
...
> 刷题模式题库。入库一题加一行（标注所属知识点）；每轮批阅后同步更新两表。
> 统计权威源是 meta.json，本表供人快速浏览。

| ID | 来源 | 类型 | 知识点 | 题干摘要 | 出现 | 答对 | 最近轮次 |
|----|------|------|--------|----------|------|------|----------|
| Q-001 | 真题 [出处: url] | 选择 | 引用语义 | <一句话摘要> | 3 | 3 | 题目-006 |
| Q-002 | AI | 填空 | 闭包 | <一句话摘要> | 2 | 1 | 题目-005 |
| Q-003 | 变种(基于Q-001·难度2) | 应用 | 引用语义 | <一句话摘要> | 0 | 0 | — |

## 知识点进度
| 知识点 | stage | 状态 | 出现/答对 | 下次出现 |
|--------|-------|------|-----------|----------|
| 引用语义 | 3 | ✅已掌握 · 复习:第12轮 | 8/7 | 题目-012 |
| 闭包 | 2 | 🔄learning | 3/2 | 随轮可选 |
```
...
- 类型: 选择 ｜ 填空 ｜ 应用 ｜ 算法
- 知识点: <所属知识点（单一、必填——选题与掌握跟踪的主键）>

## 题干
<完整题干。选择题列全 A/B/C/D 选项；填空题用 ______ 标空（多空按 ①② 编号）；算法题写清输入/输出/约束与示例>

## 答案
<选择题：字母。填空题：各空答案 + accept 同义写法列表，如 accept: ["引用", "reference"]。算法题：**可直接运行的参考实现**（完整函数或核心代码——点评时作为正确代码示例）+ 关键测试用例的期望输出；选择/填空若考点可用代码表达，附一段最小正确示例供点评引用>
...
- 题目ID: Q-xxx · 第 N 次作答
- 来源: <同题库> · 类型: <选择/填空/应用/算法> · 知识点: <知识点> · stage N

## 题目
<从题库存档复制的完整题干——本文件自包含，作答与复习无需再翻题库>

## 你的答案
<!-- 选择题写字母（多选逗号分隔）；填空题按空作答；应用题写结论+准确原因（可含代码/命令）；算法题写「见 solution.py」 -->

（在此作答）

---
*AI 批阅区（AI 批阅后填写，用户请勿改动）*
- 判定: （✓ 正确 / ✗ 错误；应用题 = 结论与原因都正确）
- 讲评: <对→一句确认或深化；错→讲清概念而非只报答案，引用题库解析；算法题→指出未通过用例与修复方向；应用题→指出原因错在哪>
- 正确代码示例: <「讲评」之后紧跟一个对应语言的代码块，规则见下方「正确示例规则」>
- 记录: 第 N 次作答 · 知识点 <名> stage <N>（<晋级/停留/退回>） · 掌握判定: <🔄learning / ✅已掌握（AI 判断理由一句） / 退回 learning（理由）> · <复习排期: 第X轮 / 错题复现: 第X轮 / 无>
```

**正确示例规则（批阅区非妥协项）**：每轮点评必附一段正确代码示例，**无论作答对错**——算法题给参考实现（答对时作为对照/更优写法，答错时给修复后的完整代码或核心片段）；选择/填空/应用给最能说明「为什么正确答案正确」的最小可运行示例（代码/命令/配置均可，如 Docker 题给正确命令、SQL 题给正确查询；应用题的示例须同时印证正确原因）。示例从题库存档的 答案 区取用（入库时即已备好），用对应语言的代码围栏呈现在「讲评」之后。仅当题目确实无任何可展示示例（罕见）才可省略并写明原因。
